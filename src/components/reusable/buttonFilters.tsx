...
type Props = {
  ...
  labels?: {
    tags?: string
    button?: (filter: FilterType) => string
  }
}
export function ProjectsButtonFilters({
  className = '',
  filter,
  onChangeFilter = () => {},
  labels = {
    button: DefaultLabelButton,
    tags: "Tags",
  },
}: Props) {
  ...
  return <div className="relative w-full md:w-auto ">
    ...
    >
      ...
      {(labels.button || DefaultLabelButton).call(null,{
        tags: selected.map(tag_name => ({ tag_name })),
      })}
    ...
    {open && <div
      ...
      role="menu"
    >
      <h6 className="mb-3 text-sm font-medium text-gray-900 dark:text-white">{labels.tags ?? "Tags"}</h6>
...
const DefaultLabelButton = (filter: FilterType) => (filter.tags && filter.tags.length > 0) ? `Filtered (${filter.tags.length == 1 ? 'one tag' : filter.tags.length + ' tags'})` : 'Filter'