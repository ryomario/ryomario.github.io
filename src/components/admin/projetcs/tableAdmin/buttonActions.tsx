...
export function ProjectsTableAdminButtonActions({
  ...
  const handleDelete = useCallback(async () => {
    const project = await RepoProjects_server.getOne(project_id)
    const confirmed = await confirm({
      title: `Delete project "${project?.project_title}"!`,
  ...
  return <>
    <button
      ref={buttonRef}
      className={[
        "inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100",
        className,
        open ? 'open' : '',
      ].join(' ')}
      type="button"
      onClick={toggleOpen}
    >
      <svg className="w-5 h-5 rotate-90" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    {open && createPortal(<div
      ...
      className={[
        "absolute mt-2",
        "z-10 w-44 bg-white border rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600",
      ].join(' ')}
      role="menu"
    >
...
function ButtonDelete({
  ...
  return <button onClick={handleDelete} type="button"
    className={`flex w-full items-center py-2 px-4 text-red-500 ${
      isLoading
      ? "bg-gray-100 cursor-not-allowed"
      : "hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-red-400"
    }`}>
    {isLoading ? <>
